...
WITH driver_metrics AS (
    SELECT 
        ...
        COUNT(DISTINCT CASE 
            WHEN issue_type = 'rider' AND issue_sub_type = 'no_mask' AND ticket_id IS NOT NULL 
            THEN ticket_id 
        END) AS rider_no_mask_complaints,
        COUNT(DISTINCT CASE 
            WHEN issue_type = 'rider' AND issue_sub_type = 'rude' AND ticket_id IS NOT NULL 
            THEN ticket_id 
        END) AS rider_rude_complaints,
        COUNT(CASE 
            WHEN issue_type = 'delivery' AND issue_sub_type = 'late' AND ticket_id IS NOT NULL 
            THEN ticket_id 
        END) AS delivery_late_complaints,
        COUNT(CASE 
            WHEN issue_type = 'delivery' AND issue_sub_type = 'not_delivered' AND ticket_id IS NOT NULL 
            THEN ticket_id 
        END) AS not_delivered_complaints,
